...
const buildAlertsApiUrl = window.__buildApiUrl || function(path = '') {
...
async function loadActiveAlerts() {
    try {
        ...
        let url = buildAlertsApiUrl(`/api/mqtt-alerts/empresa/${empresaId}/active-by-sede?limit=${alertsPerPage}&offset=${(currentPage - 1) * alertsPerPage}`);
                ...
                <div class="lg:col-span-1">
                    ${alertImageUrl ? `
                        <div class="modal-section bg-gradient-to-br ${
                            ...
                            <div class="w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 bg-gradient-to-br ${
...
async function findAlertById(alertId) {
    try {
        ...
        const response = await fetch(buildAlertsApiUrl(`/api/mqtt-alerts/${alertId}`), {
            method: 'GET',
...
function showSimpleNotification(message, type = 'info', duration = 4000) {
    ...
    notificationElement.innerHTML = `
        <div class="alert-notification fixed top-4 right-4 transform transition-all duration-500 ease-out" style="transform: translateX(100%) scale(0.8); opacity: 0; z-index: 12000;">
