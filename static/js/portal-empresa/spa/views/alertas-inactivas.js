...
const buildInactiveAlertsApiUrl = window.__buildApiUrl || function(path = '') {
            ...
            <div class="flex items-start space-x-4">
                <div class="flex-shrink-0">
                    <div class="w-16 h-16 rounded-xl flex items-center justify-center ${
                        ...
                        } text-white text-xl alert-card__icon"></i>
                    </div>
                </div>
...
async function findInactiveAlertById(alertId) {
    try {
        ...
        const response = await fetch(buildInactiveAlertsApiUrl(`/api/mqtt-alerts/${alertId}`), {
            method: 'GET',
...
function generateInactiveModalContent(alert, isUserOrigin, isHardwareOrigin, isEmpresaOrigin) {
    ...
    const alertImageUrl = typeof window.resolveAlertImageUrl === 'function'
        ? window.resolveAlertImageUrl(alert.image_alert)
        : alert.image_alert;
    ...
    return `
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div class="modal-section ${
                ...
                ${alertImageUrl ? `
                    <img src="${alertImageUrl}" alt="${alert.nombre_alerta}" 
            ...
            <div class="modal-section bg-red-600/10 border border-red-500/20 rounded-xl p-4">
                ...
                <div class="space-y-3">
                    ...
                    <div class="bg-black/20 rounded-lg p-3">
                        <span class="text-red-200 text-sm block mb-1">Motivo:</span>
                        <span class="text-white font-medium">${alert.mensaje_desactivacion || 'Sin mensaje'}</span>
                    </div>
                </div>
            </div>
        </div>
...
if (typeof window.showDeactivateConfirmation !== 'function') {
    window.showDeactivateConfirmation = function() {
        ...
        <div class="modal-section bg-teal-600/5 rounded-lg p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-48 overflow-y-auto custom-scrollbar">
                ${alert.numeros_telefonicos.sort((a, b) => {
                    ...
                    return `
                    <div class="${bgColor} rounded-lg p-3 flex items-center space-x-3">
